from color import random_hue, hue_to_color, random_color, black, change_color, gradient_wheel
    ...
    def num_alive_hexes(self, coords):
        ...
        for coord in coords:
            if self.cell_exists(coord):
                ...
                i += 0.5  # boundary cells treated as half-alive
        return i
...
class GameOfLife2(object):

    def __init__(self, hexmodel):
        self.name = "Game of Life 2"
    ...
    def next_frame(self):
        ...
        for i in range(0, 1000):  # number of steps per game
            ...
            for cell in self.life_hexes.life_map:
                if self.life_hexes.get(cell).on:  # Alive
                    if self.life_hexes.get(cell).next_on:  # -Alive
                        self.hexes.set_cell(cell, gradient_wheel(self.on_color, 1))
                    else:  # -Dead
                        self.hexes.set_cell(cell, gradient_wheel(self.off_color, 0.5))
                else:  # Dead
                    if self.life_hexes.get(cell).next_on:  # -Alive
                        self.hexes.set_cell(cell, gradient_wheel(self.on_color, 0.5))
                    else:  # -Dead
                        self.hexes.set_cell(cell, gradient_wheel(self.off_color, 1))
