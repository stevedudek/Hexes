...
class HexServer(object):
    def __init__(self, square_model, square_simulator, args):
        self.args = args
        self.square_model = square_model
        self.square_simulator = square_simulator
    ...
    def _create_services(self):
        ...
        # Show runner
        self.runner = ShowRunner(self.square_model, self.square_simulator,
    ...
    def start(self):
        if self.running:
            print "start() called, but square is already running!"
            return
    ...
    def stop(self):
        if self.running: # should be safe to call multiple times
            try:
                ...
                print "Exception stopping Hexes!"
                traceback.print_exc()
    ...
    def go_web(self, app):
        ...
        cherrypy.quickstart(HexWeb(app),
                '/',
                config=config)
...
if __name__=='__main__':
    ...
    print "Using Hex Simulator at %s:%d" % (sim_host, sim_port)

    from model.simulator import SimulatorModel
    model = SimulatorModel(sim_host, port=sim_port)
    full_squares = hex.load_squares(model)

    app = HexServer(full_squares, model, args)
    try:
        ...
        print "Unhandled exception running Hexes!"
        traceback.print_exc()
    finally:
        app.stop()
